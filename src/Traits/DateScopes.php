...
use Foundation\Support\Enums\DateRange;
use Foundation\Exceptions\InvalidArgument\InvalidTimeUnitsException;
...
{
    /**
     * Datetime column for Date Scopes
     *
     * @var string
     */
    protected string $dateScopesColumn = 'created_at';
    ...
    public function scopeOfLastUnit(
        ...
        if ($value <= 0) {
            throw new InvalidTimeUnitsException;
        }
...
//        $createdColumnName = (self::CREATED_AT != 'created_at') ? self::CREATED_AT : config('date-scopes.created_column');
        $scopesColumnName = $this->dateScopesColumn ?? config('date-scopes.created_column', self::CREATED_AT);

        return $query->whereBetween($scopesColumnName, $range);
    }
