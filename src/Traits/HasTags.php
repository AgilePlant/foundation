...
trait HasTags
{
    ...
    /**
     * Return FQCN (Full-Qualified Class Name) as type
     */
    ...
    public function attachModelTags(array | ArrayAccess | Tag $tags): static
    {
        $className = static::getTagClassName();

        $tags = collect($className::findOrCreate($tags, $this->getFQCN()));
    ...
    public function attachModelTag(string | Tag $tag)
    {
        return $this->attachTags([$tag], $this->getFQCN());
    }

    public function detachModelTags(array | ArrayAccess $tags): static
    {
        $tags = static::convertToTags($tags, $this->getFQCN());
    ...
    public function detachModelTag(string | Tag $tag): static
    {
        return $this->detachTags([$tag], $this->getFQCN());
    }

    public function syncModelTags(string | array | ArrayAccess $tags): static
    {
        if (is_string($tags)) {
            $tags = Arr::wrap($tags);
        }
        ...
        $this->syncTagIds($tags->pluck('id')->toArray(), $this->getFQCN());

        return $this;
    }

    public function scopeWithAllModelTags(
        Builder $query,
        string | array | ArrayAccess | Tag $tags,
    ): Builder {
        $type = $this->getFQCN();
    ...
    public function scopeWithAnyModelTags(
        Builder $query,
        string | array | ArrayAccess | Tag $tags,
    ): Builder {
        $type = $this->getFQCN();
